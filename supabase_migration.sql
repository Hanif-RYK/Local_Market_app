-- ============================================================
-- LocalMarket Supabase Migration
-- Run this in Supabase Dashboard → SQL Editor
-- ============================================================

-- 0. Create storage bucket for photos
...
CREATE TABLE IF NOT EXISTS service_categories (
  ...
  icon TEXT NOT NULL,
  is_free_forever INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS products (
  id BIGSERIAL PRIMARY KEY,
  shop_id BIGINT NOT NULL REFERENCES product_shops(id) ON DELETE CASCADE,
  owner_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS used_saman_categories (
  id BIGSERIAL PRIMARY KEY,
  name TEXT NOT NULL UNIQUE,
  icon TEXT NOT NULL,
  sort_order INTEGER DEFAULT 0
...
CREATE TABLE IF NOT EXISTS listing_photos (
  id BIGSERIAL PRIMARY KEY,
  listing_type TEXT NOT NULL CHECK (listing_type IN ('shop', 'property', 'product_shop', 'used_saman', 'service')),
...
-- 3. Helper function: check admin without recursion
...
-- Helper: check if user owns a record or is admin
...
-- 4. Profiles RLS policies (non-recursive)
-- Profile is special: we must NOT query profiles inside a profiles policy
...
-- 5. Auto-create profile on signup (avoids need for manual insert)
...
-- 6. RLS for content tables: owners can CRUD own, anyone can SELECT active

-- Services
DROP POLICY IF EXISTS "Anyone can view active services" ON services;
CREATE POLICY "Anyone can view active services" ON services
  FOR SELECT USING (status = 'active' OR public.is_owner_or_admin(owner_id));

DROP POLICY IF EXISTS "Owners insert services" ON services;
CREATE POLICY "Owners insert services" ON services
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners update own services" ON services;
CREATE POLICY "Owners update own services" ON services
  FOR UPDATE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners delete own services" ON services;
CREATE POLICY "Owners delete own services" ON services
...
DROP POLICY IF EXISTS "Owners delete own shops" ON shops;
CREATE POLICY "Owners delete own shops" ON shops
  FOR DELETE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Admin manage shops" ON shops;
CREATE POLICY "Admin manage shops" ON shops
  FOR ALL USING (public.is_admin());
...
DROP POLICY IF EXISTS "Owners insert properties" ON properties;
CREATE POLICY "Owners insert properties" ON properties
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners update own properties" ON properties;
CREATE POLICY "Owners update own properties" ON properties
  FOR UPDATE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners delete own properties" ON properties;
...
CREATE POLICY "Anyone can view active product_shops" ON product_shops
  FOR SELECT USING (status = 'active' OR public.is_owner_or_admin(owner_id));

DROP POLICY IF EXISTS "Owners insert product_shops" ON product_shops;
CREATE POLICY "Owners insert product_shops" ON product_shops
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners update own product_shops" ON product_shops;
CREATE POLICY "Owners update own product_shops" ON product_shops
  FOR UPDATE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners delete own product_shops" ON product_shops;
CREATE POLICY "Owners delete own product_shops" ON product_shops
  FOR DELETE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Admin manage product_shops" ON product_shops;
CREATE POLICY "Admin manage product_shops" ON product_shops
  FOR ALL USING (public.is_admin());
...
CREATE POLICY "Owners insert products" ON products
  FOR INSERT WITH CHECK (auth.uid() = owner_id);
...
CREATE POLICY "Anyone can view active used_saman" ON used_saman
  FOR SELECT USING (status = 'active' OR public.is_owner_or_admin(owner_id));

DROP POLICY IF EXISTS "Owners insert used_saman" ON used_saman;
CREATE POLICY "Owners insert used_saman" ON used_saman
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners update own used_saman" ON used_saman;
CREATE POLICY "Owners update own used_saman" ON used_saman
  FOR UPDATE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Owners delete own used_saman" ON used_saman;
CREATE POLICY "Owners delete own used_saman" ON used_saman
  FOR DELETE USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Admin manage used_saman" ON used_saman;
CREATE POLICY "Admin manage used_saman" ON used_saman
  FOR ALL USING (public.is_admin());

-- Listing Photos
DROP POLICY IF EXISTS "Anyone can view listing_photos" ON listing_photos;
CREATE POLICY "Anyone can view listing_photos" ON listing_photos
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated insert listing_photos" ON listing_photos;
CREATE POLICY "Authenticated insert listing_photos" ON listing_photos
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Owners delete listing_photos" ON listing_photos;
CREATE POLICY "Owners delete listing_photos" ON listing_photos
  FOR DELETE USING (
    EXISTS (
...
-- Contact Logs (insert only, view by admin)
DROP POLICY IF EXISTS "Anyone insert contact_logs" ON contact_logs;
CREATE POLICY "Anyone insert contact_logs" ON contact_logs
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Admin view contact_logs" ON contact_logs;
CREATE POLICY "Admin view contact_logs" ON contact_logs
  FOR SELECT USING (public.is_admin());
...
DROP POLICY IF EXISTS "Admin view reports" ON reports;
CREATE POLICY "Admin view reports" ON reports
  FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Admin delete reports" ON reports;
CREATE POLICY "Admin delete reports" ON reports
  FOR DELETE USING (public.is_admin());

-- Payment Requests
DROP POLICY IF EXISTS "Users view own payment_requests" ON payment_requests;
CREATE POLICY "Users view own payment_requests" ON payment_requests
  FOR SELECT USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Users insert payment_requests" ON payment_requests;
CREATE POLICY "Users insert payment_requests" ON payment_requests
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Admin view payment_requests" ON payment_requests;
CREATE POLICY "Admin view payment_requests" ON payment_requests
  FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Admin manage payment_requests" ON payment_requests;
CREATE POLICY "Admin manage payment_requests" ON payment_requests
  FOR ALL USING (public.is_admin());

-- Plans
DROP POLICY IF EXISTS "Users view own plans" ON plans;
CREATE POLICY "Users view own plans" ON plans
  FOR SELECT USING (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Users insert own plans" ON plans;
CREATE POLICY "Users insert own plans" ON plans
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Admin view all plans" ON plans;
CREATE POLICY "Admin view all plans" ON plans
  FOR SELECT USING (public.is_admin());

-- App Settings (public read, admin write)
DROP POLICY IF EXISTS "Anyone view app_settings" ON app_settings;
CREATE POLICY "Anyone view app_settings" ON app_settings
  FOR SELECT USING (true);
...
INSERT INTO service_categories (name, icon, is_free_forever, sort_order) VALUES
  ('Mazdoor / Labour', '👷', 1, 1),
  ('Mistri / Construction', '🔨', 0, 2),
  ('Plumber', '💧', 0, 3),
  ('Electrician', '⚡', 0, 4),
  ('Carpenter', '🪟', 0, 5),
  ('Painter', '🎨', 0, 6),
  ('AC Mechanic', '❄️', 0, 7),
  ('General Mechanic', '🔧', 0, 8),
  ('Welder', '🔩', 0, 9),
  ('Tile / Floor Work', '🏗️', 0, 10),
  ('Roof Work', '🏠', 0, 11),
  ('Other Service', '🛠️', 0, 12)
...
-- Seed admin user (id matches a real auth user to be created manually)
-- After you create this user in Supabase Auth UI, run:
-- INSERT INTO profiles (id, name, role) VALUES ('<ACTUAL_AUTH_USER_UUID>', 'Admin', 'admin') ON CONFLICT (id) DO UPDATE SET role = 'admin';
-- Or use the trigger which auto-creates profiles on signup, then:
-- UPDATE profiles SET role = 'admin' WHERE id = '<ACTUAL_AUTH_USER_UUID>';
